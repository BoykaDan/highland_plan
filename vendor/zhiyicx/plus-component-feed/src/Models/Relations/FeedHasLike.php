...
trait FeedHasLike
{
    ...
    public function like($user)
    {
        ...
        return $this->getConnection()->transaction(function () use ($user, $user_name) {
            ...
            // 发送用户通知
            $this->user->notify(new \Zhiyi\Plus\Notifications\UserNotification(
                (new \Zhiyi\Plus\Notifications\Messages\UserNotificationMessage(
                    'feed:digg',
                    sprintf('@%s 点赞了你的动态', $user_name),
                    ['user_id' => $user]
