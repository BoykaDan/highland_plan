...
class FeedController extends Controller
{
    ...
    public function getPinnedFeeds(Request $request, FeedModel $feedModel, FeedRepository $repository)
    {
        if ($request->query('after')) {
            return [];
        }

        $feeds = $feedModel->where('pinned', 1)
            ->orderBy('id', 'desc')
            ->get();
    ...
    public function hot(Request $request, LikeModel $model, FeedRepository $repository, Carbon $dateTime)
    {
        ...
        $feeds = FeedModel::whereIn('id', $ids)->get();
     ...
     * @return mixed
     */
    public function users(Request $request, FeedModel $feedModel, FeedRepository $repository)
    {
        $user = $request->user('api')->id ?? 0;
        $limit = $request->query('limit', 20);
        $after = $request->query('after');
